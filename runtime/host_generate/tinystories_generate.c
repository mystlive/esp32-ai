...
static void matvec_head_omp4(const QT *t, const float *x, float *y) {
    static int8_t xq[LLM_Q8_MAX_INPUT];
    float xs;

    if (t->w8 == NULL || t->rows < 128) {
        MATVEC(t, x, y);
        return;
    }
    ...
    #pragma omp parallel num_threads(4)
    {
        ...
        int begin = (t->rows * tid) / nth;
        int end   = (t->rows * (tid + 1)) / nth;

        matvec_i8_range(t, xq, xs, y, begin, end);
    }
}
...
int main(int argc, char **argv) {
    ...
    model.head_matvec = matvec_head_omp4;
