...
#define LLM_PROFILE_NOW() profile_now_us()
...
int main(int argc, char **argv) {
    ...
    double prof_total =
        (double)s.profile.input_us +
        (double)s.profile.attn_us +
        (double)s.profile.ffn_us +
        (double)s.profile.ple_us +
        (double)s.profile.head_us;

    printf("\n--- profile ---\n");
    printf("profile calls: %u\n", s.profile.calls);

#define PRINT_PROFILE(name, field) do {                                      \
        double us = (double)s.profile.field;                                 \
        printf("%-10s %7.3f ms/token  %5.1f %%\n",                          \
               name,                                                         \
               us / 1000.0 / s.profile.calls,                               \
               prof_total > 0.0 ? us * 100.0 / prof_total : 0.0);            \
    } while (0)

    PRINT_PROFILE("input", input_us);
    PRINT_PROFILE("attention", attn_us);
    PRINT_PROFILE("FFN", ffn_us);
    PRINT_PROFILE("PLE", ple_us);
    PRINT_PROFILE("head", head_us);

#undef PRINT_PROFILE

    printf("profiled   %7.3f ms/token\n",
           prof_total / 1000.0 / s.profile.calls);
